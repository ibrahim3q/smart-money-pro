...
const KEYS = {
  ...
  TRADE_ARCHIVE: 'auto_trade:trade_archive', // ✨ الأرشيف الدائم لكل الصفقات المغلقة (لا ينتهي)
};
...
// ── ✨ حظر يومي: سهم أُغلق بخسارة لا يُعاد دخوله لبقية اليوم ──
// إشارته فشلت مرة اليوم — ظروفه غالباً لم تتغير خلال ساعة، وإعادة
// المطاردة تكرار موثّق للخسارة (PLTR: وقف −$12 ثم إعادة دخول بنفس
// الإشارة تقريباً بنفس اليوم). المفتاح مرتبط بتاريخ اليوم فينتهي تلقائياً.
async function setDayBlock(ticker) {
  const r = getClient();
  await r.set(`auto_trade:day_block:${todayKeyET()}:${ticker}`, '1', 'EX', 172800);
}
async function isDayBlocked(ticker) {
  const r = getClient();
  const val = await r.get(`auto_trade:day_block:${todayKeyET()}:${ticker}`);
  return val !== null;
}
...
async function logDecision(entry) {
  ...
  await r.ltrim(KEYS.DECISION_LOG, 0, 1999); // ✨ رُفع السقف من 500 — يغطي ~أسبوعاً من القرارات
}
...
async function recordDailyTrade(trade) {
  const r = getClient();
  const record = { ...trade, date: todayKeyET(), recordedAt: new Date().toISOString() };
  const key = KEYS.DAILY_TRADES(todayKeyET());
  await r.rpush(key, JSON.stringify(record));
  await r.expire(key, 172800);
  // ── ✨ الأرشيف الدائم: نفس الصفقة تُحفظ بلا انتهاء (حتى 5000 صفقة =
  // سنوات من التاريخ) — هذا ما يمنع تبخر بيانات التقييم كل 48 ساعة ──
  await r.rpush(KEYS.TRADE_ARCHIVE, JSON.stringify(record));
  await r.ltrim(KEYS.TRADE_ARCHIVE, -5000, -1);
}
...
// ── ✨ قراءة الأرشيف الدائم (الأحدث أولاً) ──
async function getArchivedTrades(limit = 1000) {
  const r = getClient();
  const capped = Math.min(Math.max(1, limit), 5000);
  const raw = await r.lrange(KEYS.TRADE_ARCHIVE, -capped, -1);
  return raw
