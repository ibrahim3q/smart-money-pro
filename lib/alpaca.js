...
// ── ✨ تقويم السوق الرسمي: أيام التداول وأوقات الفتح/الإغلاق الفعلية ──
// يغطي العطل وأيام الإغلاق المبكر لأي سنة — يرجع مصفوفة مثل:
// [{ date: '2026-12-24', open: '09:30', close: '13:00' }]
// اليوم الغائب من النتيجة = السوق مغلق فيه كلياً.
async function fetchMarketCalendar(startDate, endDate) {
  return alpacaFetch(`/v2/calendar?start=${startDate}&end=${endDate || startDate}`);
}
...
// ✨ clientOrderId (اختياري): معرّف ثابت يمنع تكرار نفس الأمر — لو أُعيدت
// المحاولة بعد انقطاع شبكة غامض (الطلب وصل لكن الرد ضاع)، Alpaca ترفض
// المعرّف المكرر بدل فتح مركز مضاعف.
async function openEquityLimitOrder({ symbol, qty, limitPrice, clientOrderId }) {
  const body = {
    symbol: symbol.toUpperCase(),
    qty: String(qty),
    side: 'buy',
